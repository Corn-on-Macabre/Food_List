...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className={`${CARD_SURFACE} p-4 shadow-sm`}>
      <h2 className="font-display text-lg font-bold text-brand-text mb-3">{title}</h2>
      {children}
    </section>
  );
}
...
function Tile({ label, value }: { label: string; value: string }) {
  return (
    <div className={`${CARD_SURFACE} p-4 shadow-sm text-center`}>
      ...
      <p className="font-sans text-xs text-brand-text-muted mt-1">{label}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-brand-bg">
      <div className="max-w-3xl mx-auto px-4 py-6 flex flex-col gap-4">
        <header className="flex items-baseline justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-brand-text">Visit Stats</h1>
            <p className="font-sans text-xs italic text-brand-text-faint">private — only you can see this</p>
          </div>
          <Link
            to="/"
        ...
        {!session && (
          <div className={`${CARD_SURFACE} p-6 shadow-sm text-center`}>
            ...
            <button
              ...
              className="bg-brand-cta hover:bg-brand-cta-hover text-brand-on-accent font-sans text-sm font-bold rounded-lg px-6 py-2.5 transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-focus"
            >
              Sign in with Google
            </button>
          </div>
        ...
        {session && error && (
          <div className={`${CARD_SURFACE} p-6 shadow-sm text-center`}>
            <p className="font-sans text-sm text-brand-text-muted">Couldn't load visits: {error}</p>
          </div>
        )}

        {session && !error && (!stats || restaurantsLoading) && (
          <p className="font-sans text-sm text-brand-text-muted text-center py-8 animate-pulse">tallying the damage&hellip;</p>
        )}
          ...
          <>
            ...
            <Section title="By month">
              {stats.months.length === 0 ? (
                <p className="font-sans text-sm text-brand-text-muted">No visits logged yet.</p>
              ) : (
                <table className="w-full font-sans text-sm">
                  <tbody>
                    {stats.months.map((m) => (
                      <tr key={m.month} className="border-b border-brand-border-light last:border-0">
                        <td className="py-1.5 text-brand-text">{formatMonth(m.month)}</td>
                        <td className="py-1.5 text-right text-brand-text-muted">{m.count} visit{m.count === 1 ? '' : 's'}</td>
                        <td className="py-1.5 text-right text-brand-text font-semibold w-28">
                  ...
                  <tbody>
                    {stats.cuisines.map((c) => (
                      <tr key={c.cuisine} className="border-b border-brand-border-light last:border-0">
                        <td className="py-1.5 text-brand-text">{c.cuisine}</td>
                        <td className="py-1.5 text-right text-brand-text-muted">{c.count} visit{c.count === 1 ? '' : 's'}</td>
                        <td className="py-1.5 text-right text-brand-text font-semibold w-28">
                  ...
                  <tbody>
                    {stats.top.map((r) => (
                      <tr key={r.restaurantId} className="border-b border-brand-border-light last:border-0">
                        <td className="py-1.5 text-brand-text">
                          <Link to={`/r/${r.restaurantId}`} className="hover:underline">{r.name}</Link>
                        </td>
                        <td className="py-1.5 text-right text-brand-text-muted">{r.count}&times;</td>
                        <td className="py-1.5 text-right text-brand-text-muted w-28">{r.lastVisit}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <Section title="Loved, but it's been a while (6+ months)">
              {stats.stale.length === 0 ? (
                <p className="font-sans text-sm text-brand-text-muted">You're keeping up with all your favorites. Respect.</p>
              ) : (
                <table className="w-full font-sans text-sm">
                  <tbody>
                    {stats.stale.map((e) => (
                      <tr key={e.restaurant.id} className="border-b border-brand-border-light last:border-0">
                        ...
                        <td className="py-1.5 text-right text-brand-text-muted">
