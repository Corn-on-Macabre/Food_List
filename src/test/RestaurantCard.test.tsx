...
describe("RestaurantCard", () => {
  describe("Google Maps link (AC 1, 2, 3, 4)", () => {
    ...
    it("uses the correct googleMapsUrl for a different restaurant (AC 4)", () => {
      render(<RestaurantCard restaurant={mockRestaurantNoNotes} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
      const link = screen.getByRole("link", { name: "Open in Google Maps" });
    ...
    it("falls back to '#' href for non-http(s) URLs to prevent XSS (security)", () => {
      ...
      render(<RestaurantCard restaurant={maliciousRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
      const link = screen.getByRole("link", { name: "Open in Google Maps" });
  ...
  describe("Bobby's Pick badge (AC 9)", () => {
    ...
    it("renders BobbyPickBadge when featured is true", () => {
      ...
      render(<RestaurantCard restaurant={featuredRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
  ...
  describe("dismissal (AC 1, 4, 5)", () => {
    it("calls onDismiss when close button is clicked (AC 1)", () => {
      ...
      render(<RestaurantCard restaurant={mockRestaurant} onDismiss={onDismiss} onShareSuccess={noop} filterBarHeight={120} />);
    ...
    it("root div stops propagation to prevent map background click handler (AC 2, 3)", () => {
      ...
      render(
        <div onClick={parentHandler}>
          <RestaurantCard restaurant={mockRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />
        </div>,
      );
    ...
    it("clicking close button fires onDismiss once and does not bubble to parent", () => {
      ...
      render(
        <div onClick={parentHandler}>
          <RestaurantCard restaurant={mockRestaurant} onDismiss={onDismiss} onShareSuccess={noop} filterBarHeight={120} />
        </div>,
      );
      fireEvent.click(screen.getByRole("button", { name: "Close restaurant card" }));
      expect(onDismiss).toHaveBeenCalledTimes(1);
  ...
  describe("Rating and price display (Story 5.2)", () => {
    ...
    it("renders only rating (no price section) when priceLevel is absent (AC 5)", () => {
      render(<RestaurantCard restaurant={mockRatingOnlyRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
    ...
    it("renders only price (no rating section) when rating is absent (AC 4)", () => {
      render(<RestaurantCard restaurant={mockPriceOnlyRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
      expect(screen.getByText("$$$")).toBeInTheDocument();
      expect(screen.queryByText("★")).not.toBeInTheDocument();
      expect(screen.queryByText("·")).not.toBeInTheDocument();
    });
    ...
    it("renders rating without count when userRatingCount is absent (AC 3)", () => {
      render(<RestaurantCard restaurant={mockRatingNoCountRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
    ...
    it("does not render price for unknown priceLevel value like PRICE_LEVEL_FREE (AC 8)", () => {
      ...
      render(<RestaurantCard restaurant={unknownPriceRestaurant} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
      expect(screen.queryByText("$")).not.toBeInTheDocument();
  ...
  describe("Restaurant photo display (Story 5.3)", () => {
    ...
    it("renders photo when photoRef is present (AC 1)", () => {
      render(<RestaurantCard restaurant={mockRestaurantWithPhoto} onDismiss={noop} onShareSuccess={noop} filterBarHeight={120} />);
